...
class ImageQualityAssessor:
    ...
    def _assess_noise_level_wavelet(self, gray_image: np.ndarray) -> float:
        ...
        try:
            ...
            return float(max(0.0, min(1.0, np.std(gray_image) / 50.0)))
...
def assess_image_metrics(input_data: str) -> str:
    ...
    try:
        ...
        if output is not None:
            ...
            return json.dumps({"error": "Assessment failed"})
...
# Pydantic models are now imported from schemas.image_processing
    ...
    try:
        ...
        if input_obj.method == "unsharp_mask":
            ...
            if input_obj.threshold > 0:
                _, laplacian = cv2.threshold(laplacian.astype(np.uint8), input_obj.threshold, 255, cv2.THRESH_BINARY)  # type: ignore
            
            # Convert back to 3-channel and apply
            laplacian_3ch = cv2.cvtColor(laplacian.astype(np.uint8), cv2.COLOR_GRAY2BGR)  # type: ignore
    ...
    try:
        ...
        if input_obj.color_enhancement != "none":
            if input_obj.color_enhancement == "saturation":
                ...
                means = cv2.mean(enhanced)  # type: ignore
                b_mean, g_mean, r_mean = means[0], means[1], means[2]  # type: ignore
